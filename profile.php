...
// Fetch absolute updated details for the logged-in profile dashboard user
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($new_username) || empty($new_email)) {
        ...
        // Validate if username already taken by another account using PDO
        ...
        if ($check_stmt->fetch()) {
            ...
            if (!empty($new_password)) {
                ...
                $execute_status = $update_stmt->execute([$new_username, $new_email, $hashed_password, $user_id]);
            } else {
                $update_stmt = $conn->prepare("UPDATE users SET username = ?, email = ? WHERE id = ?");
                $execute_status = $update_stmt->execute([$new_username, $new_email, $user_id]);
            }
